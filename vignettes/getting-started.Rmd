...
- **`generate()`** makes one or a few constrained calls and reconstructs
  per-label logprobs from the winning generation path using a prefix
  trie. It is fast and budget-controlled, but the result may be
  *approximate* when the budget runs out before every label is fully
  resolved.
...
The `max_calls` argument controls `generate()`'s API budget:

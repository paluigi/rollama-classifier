...
- **`generate()`** — Hierarchical constrained generation. A single
  ...
  probability mass *within* a cluster — never changing between-group totals,
  so accuracy never degrades as the call budget grows.
...
The first call always constrains the model to *all* labels and produces
an internally consistent probability distribution. Supplementary calls
(when `max_calls > 1`) only **reproportion** probability mass *within*
clusters of unresolved labels — they never change between-group totals,
so increasing `max_calls` can only improve or leave unchanged the
prediction, never degrade it.

...
# (excluded via .Rbuildignore). It exercises a running local vLLM instance
# and the Qwen/Qwen2.5-3B-Instruct model through both scoring methods of
# llm_classifier():
#
...
# Run with:
#
#     Rscript local_tests/test_local_vllm.R
#
# The whole script skips automatically if vLLM is unreachable or the model
...
model_present <- function(model = MODEL, host = HOST, port = PORT) {
  tryCatch({
    ...
    any(grepl(model, ids, fixed = TRUE))
  }, error = function(e) FALSE)
}

skip_if_no_server <- function() {
  if (!port_open(host = HOST, port = PORT)) {
    skip(paste0("vLLM server not reachable at ", HOST, ":", PORT))
  }
}

skip_if_no_model <- function() {
  if (!model_present()) {
    skip(paste0("Model '", MODEL, "' not found on the vLLM server"))
  }
}

# Build the classifier
make_classifier <- function() {
  backend <- vllm_backend(model = MODEL, base_url = BASE_URL)

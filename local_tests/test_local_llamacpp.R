...
# (excluded via .Rbuildignore). It exercises a running local llama.cpp
# instance and the qwen2.5-3b-instruct-gguf model through both scoring
...
# Run with:
#
#     Rscript local_tests/test_local_llamacpp.R
#
# The whole script skips automatically if the server is unreachable or the
# model is not present, so sourcing it elsewhere never hard-fails.
...
model_present <- function(host = HOST, port = PORT) {
  ...
  tryCatch({
    ...
    models <- resp$models %||% list()
    names <- vapply(models, function(m) {
      m$name %||% m$model %||% ""
    }, character(1))
    any(grepl("qwen2.5-3b-instruct", tolower(names), fixed = TRUE))
  }, error = function(e) FALSE)
}

skip_if_no_server <- function() {
  if (!port_open(host = HOST, port = PORT)) {
    skip(paste0("llama.cpp server not reachable at ", HOST, ":", PORT))
  }
}

skip_if_no_model <- function() {
  if (!model_present()) {
    skip(paste0("Model '", MODEL, "' not found on the llama.cpp server"))
  }
}

# Build the classifier
make_classifier <- function() {
  backend <- llamacpp_backend(model = MODEL, base_url = BASE_URL)
